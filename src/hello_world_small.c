...
int main() //BEGIN
{
    printf("TESTE DE BLINK LEDS\n"); //exibe no console (jtag_uart)
    // pisca todos os 18 leds vermelhos 6 vezes
    for(i=0; i<6; i++) {
    ...
    // REPETE ATÉ DESLIGAR ACENDENDO UM POR UM DO LSB ao MSB
    while (1) {
    	for(i=0;i<8;i++) {
    		IOWR_ALTERA_AVALON_PIO_DATA(LEDS_R_BASE, led[i]);
    		usleep(500000);
    	}
